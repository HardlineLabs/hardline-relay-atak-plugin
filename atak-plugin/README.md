...
with SwiftShader graphics. The plugin also loads and binds to real radios on both
phones; supported behavior and the field-testing boundary are recorded in
...
2. Install the SDK host with `./tools/devices.ps1 install --apk "$env:USERPROFILE/HardlineRelayDev/references/ATAK-CIV-5.6.0.23-SDK/ATAK-CIV-5.6.0.23-SDK/atak.apk"`.
...
4. Open ATAK, complete local permission/onboarding prompts, enable Hardline Relay
   in Plugins, then open its toolbar item. Open its channel and mesh-status pane.
