...
internal class ChatSharing(
    ...
    companion object {
        ...
        fun connector() =
            object : PluginConnector(SEND) {
                override fun getConnectionLabel() = "Relay chat"

                override fun getIconUri() =
                    "android.resource://android/${android.R.drawable.sym_action_chat}"
            }
    }
    ...
    var status = "Chat · Select a Relay contact in ATAK chat"
        private set
    ...
    private val db
        get() = ChatDatabase.getInstance(map.context)

    private val contactHandler =
        object : ContactConnectorManager.ContactConnectorHandler() {
            ...
            override fun getFeature(
                type: String,
                feature: ContactConnectorManager.ConnectorFeature,
                uid: String,
                address: String,
            ): Any? = null
            ...
            override fun handleContact(type: String, uid: String, address: String): Boolean {
                ...
                val target =
                    Contacts.getInstance().getContactByUuid(uid) as? IndividualContact
                        ?: return false
    ...
    private val receiver =
        object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                try {
                    val bundle =
                        intent.getBundleExtra(ChatManagerMapComponent.PLUGIN_SEND_MESSAGE_EXTRA)
                            ?: return
                    check(localNode() != null) {
                        "Select a connected private Relay channel before chatting."
                    }
                    val destinations = bundle.getStringArray("destinations").orEmpty()
                    check(destinations.size == 1) {
                        "Choose one Relay contact for each chat. Public rooms and group fan-out are not supported."
                    }
                    ...
                    val contact =
                        Contacts.getInstance().getContactByUuid(uid) as? IndividualContact
                            ?: error(
                                "Relay contact unavailable. Receive fresh PLI on this channel first."
                            )
                    ...
                    val message =
                        ChatWire.Text(
                            ...
                            callsign(),
                            requireNotNull(bundle.getString("message")),
                        )
                    val bytes = ChatWire.encode(message)
                    val now = SystemClock.elapsedRealtime()
                    pending.entries.removeAll { now - it.value.at > 300_000 }
                    check(pending.size < 32) {
                        "Relay chat has 32 recent messages. Wait before sending more."
                    }
                    check(id !in pending) {
                        "This message is already tracked. Check its delivery status."
                    }
                    ...
                    transmit(bytes) { error ->
                        if (error != null && pending[id]?.confirmed != true) {
                            pending.remove(id)
                            status = "Chat not submitted · $error"
    ...
    private fun callsign(): String {
        ...
        while (name.toByteArray(Charsets.UTF_8).size > 24) name =
    ...
    fun start() {
        ...
        AtakBroadcast.getInstance()
            .registerReceiver(
                receiver,
                AtakBroadcast.DocumentedIntentFilter(SEND, "Send ATAK chat to a Relay contact"),
            )
        started = true
    }
    ...
    fun reset() {
        ...
        receipts.clear()
        lastProof = -1
        status = "Chat · Select a Relay contact in ATAK chat"
    }
    ...
    fun receive(from: String, bytes: ByteArray, now: Long) {
        ...
        when (message) {
            is ChatWire.Receipt -> {
                ...
                sent.confirmed = true
                val bundle =
                    Bundle(sent.bundle).apply {
                        putString("status", "DELIVERED")
                        putLong("receiveTime", System.currentTimeMillis())
                    }
                persist(bundle)
            ...
            is ChatWire.Text -> {
                ...
                if (db.getChatMessage(id) == null) {
                    ...
                    val bundle =
                        Bundle().apply {
                            putString("conversationId", uid)
                            putString("conversationName", "${message.callsign} [Relay]")
                            putString("messageId", id)
                            putString("senderUid", uid)
                            putString("senderCallsign", message.callsign)
                            putString("uid", self)
                            putStringArray("destinations", arrayOf(self))
                            putString("parent", "RootContactGroup")
                            putString("status", "NONE")
                            putString("message", message.text)
                            putLong("sentTime", message.time)
                            putLong("receiveTime", System.currentTimeMillis())
                        }
                    persist(bundle)
                }
    ...
    private fun persist(bundle: Bundle) {
        ...
        AtakBroadcast.getInstance()
            .sendBroadcast(
                Intent("com.atakmap.android.chat.NEW_CHAT_MESSAGE").apply {
