...
class RelayPlugin(services: IServiceController) : IPlugin {
    ...
    private val button =
        ToolbarItem.Builder(
                ...
                MarshalManager.marshal(
                    pluginContext.getDrawable(R.drawable.ic_hardline),
    ...
    private fun read(s: IMeshService): Snapshot {
        ...
        check(s.myNodeInfo?.firmwareVersion == "2.7.15.567b8ea") {
            "Radio firmware must be 2.7.15.567b8ea."
        }
        val config = LocalConfig.ADAPTER.decode(s.config)
        check(config.lora?.tx_enabled == true) {
            "Radio transmission is disabled. Enable Transmit in Meshtastic."
        }
        check(
            textTransport ||
        ...
        check(config.lora?.hop_limit == 7) {
            "Expected 7 hops. Inspect Meshtastic; no settings changed."
        }
        return Snapshot(
            s.myId,
            ChannelSet.ADAPTER.decode(s.channelSet).settings,
            7,
            requireNotNull(config.lora),
        )
    }

    private fun privateChannel(c: ChannelSettings) =
        c.psk.size == 32 &&
            c.name.isNotBlank() &&
            !c.name.equals("admin", true) &&
            !c.uplink_enabled &&
            !c.downlink_enabled
    ...
    private val connection =
        object : ServiceConnection {
            ...
            override fun onServiceDisconnected(name: ComponentName) {
                ...
                status = "Radio reconnecting · Reporting will resume on the same channel."
            }

            override fun onBindingDied(name: ComponentName) {
                disconnect()
                radioAvailable = false
                status = "Reconnecting to Meshtastic…"
            }

            override fun onNullBinding(name: ComponentName) {
                disconnect()
                radioAvailable = false
                status = "Meshtastic unavailable · Retrying connection…"
            }
        }
    ...
    private fun connect() {
        ...
        try {
            ...
            status = "Meshtastic binding failed. Check installed version and permissions."
        }
    }
            ...
            override fun onReceive(context: Context, intent: Intent) {
                ...
                if (intent.action == "com.geeksville.mesh.MESSAGE_STATUS") {
                    ...
                    if (radioPackets.containsKey(id)) {
                        @Suppress("DEPRECATION")
                        val value =
                            intent.getParcelableExtra<org.meshtastic.core.model.MessageStatus>(
                                "com.geeksville.mesh.Status"
                            )
                ...
                try {
                    ...
                    val packet =
                        intent.getParcelableExtra<DataPacket>("com.geeksville.mesh.Payload")
                            ?: return
                    if (
                        packet.dataType !in listOf(PliWire.PORT, 1) ||
                            packet.channel != session.selected ||
                            packet.viaMqtt
                    )
                        return
                    val from = packet.from?.lowercase(Locale.ROOT) ?: return
                    if (!from.matches(Regex("![0-9a-fA-F]{8}")) || from == session.snapshot?.node)
                        return
                    val raw = packet.bytes?.toByteArray() ?: return
                    val bytes =
                        if (packet.dataType == 1) TransportWire.decodeText(raw) ?: return else raw
                    privateReceived++
                    ...
                    if (ChatWire.isChat(bytes)) {
                        ...
                        if (
                            m is ChatWire.Receipt &&
                                m.recipient == session.snapshot?.node?.let(PointWire::nodeId)
                        )
                            delivery.acknowledge("chat:${m.id}", from)
                        return
                    }
                    if (PointWire.isPointMessage(bytes)) {
                        ...
                        if (
                            m is PointWire.Receipt &&
                                m.recipient == session.snapshot?.node?.let(PointWire::nodeId)
                        )
                            delivery.acknowledge("point:${m.token}", from)
                        if (
                            previous != PointSendState.Status.RECEIVED &&
                                points.last.status == PointSendState.Status.RECEIVED
                        )
                            lastPointProof = now
                        render()
                        return
                    ...
                    when (message) {
                        is PliWire.Receipt ->
                            if (state.receipt(message.id, from, now)) {
                        ...
                        is PliWire.Position -> {
                            if (state.accept(from, message.pli, now, System.currentTimeMillis())) {
                                ...
                                receipts.enqueue(
                                    ...
                                    message.pli,
                                    now,
                                    random.nextInt(2501).toLong() + 500,
                                )
                            } else if (state.lastReception == PliState.Reception.DUPLICATE) {
                                // Duplicate position must not refresh the marker, but can recover a
                                // lost receipt.
                                if (markers.containsKey(from))
                                    receipts.enqueue(
                                        from,
                                        message.pli,
                                        now,
                                ...
                                rejected++
                                lastRejection =
                                    "Position ${state.lastReception.name.lowercase(Locale.ROOT)}"
                            }
                        }
                    }
                    render()
                    ...
                    rejected++
                    lastRejection = "Unsupported or malformed private packet"
                }
            }
        }
    ...
    ) // Pre-33 branch; exported required for Meshtastic IPC.
    override fun onStart() {
        ...
        val filter =
            IntentFilter("com.geeksville.mesh.RECEIVED.PRIVATE_APP").apply {
                addAction("com.geeksville.mesh.MESSAGE_STATUS")
                addAction("com.geeksville.mesh.RECEIVED.TEXT_MESSAGE_APP")
            }
        if (Build.VERSION.SDK_INT >= 33)
            host.registerReceiver(receiver, filter, Context.RECEIVER_EXPORTED)
    ...
    private fun clearPeers() {
        ...
        rejected = 0
        lastRejection = "None"
        radioStatus = "No radio submission"
        chat.reset()
    }
    ...
    private val tick =
        object : Runnable {
            override fun run() {
                ...
                if (
                    (manualRequested || interval > 0 && now >= nextSend) &&
                        radioAvailable &&
                        !session.busy
                )
                    sendPli()
                if (session.selected != null && !session.busy)
                    receipts.poll(now)?.let {
                        transmit(PliWire.ack(it), DataPacket.ID_BROADCAST, null)
                    }
                if (
                    radioAvailable &&
                        !profileSwitching &&
                    ...
                    delivery.poll(now)?.let(::sendFrame)
                state.expire(now)
                updateMarkers(now)
                render()
                    ...
                    .onSuccess { current ->
                        ...
                        catalogIssue =
                            if (catalog.isFailure)
                                "Open Hardline Relay to reconnect saved channels. Sending paused."
                            else null
                        ...
                        if (old != null && old != current)
                            reset(
                                "Radio/channel changed. Select channel again; automatic sending stopped."
                            )
                        val changed = session.observe(current)
                        radioAvailable = true
                        if (changed || profilesChanged) {
                            status =
                                "Radio connected (${current.node}), 7 hops. Select a private channel."
                            rebuildChannels()
                        }
                        if (profileSwitching) {
                            session.select(null)
                            ...
                            status =
                                catalogIssue
                                    ?: "Radio activation in progress or unverified. Open Relay for status."
                            rebuildChannels()
                        } else
                            profiles
                                .firstOrNull {
                                    it.activation.isNotEmpty() &&
                                        it.activation != lastActivation &&
                                        matchesActive(it)
                                }
                                ?.let { entry ->
                                    lastActivation = entry.activation
                                    chooseChannel(entry.index)
                                }
                    }
                    .onFailure {
                        radioAvailable = false
                        status =
                            (it.message ?: "Radio unavailable.") +
                                " · Waiting to reconnect; last-known contacts retained."
                    }
                render()
            }
        }
    ...
    private fun positionFix(): Pair<GeoPoint, android.location.Location> {
        ...
        check(
            host.checkSelfPermission(android.Manifest.permission.ACCESS_FINE_LOCATION) ==
        ...
        val fix =
            lm.getLastKnownLocation(LocationManager.GPS_PROVIDER)?.takeIf {
                it.elapsedRealtimeNanos > 0 &&
                    (SystemClock.elapsedRealtimeNanos() - it.elapsedRealtimeNanos) in
                        0..120_000_000_000L
        ...
        check(distance[0] <= 100) {
            "ATAK self position differs from current phone fix. Wait for ATAK location."
        }
        return point to fix
    }
    ...
    private fun sendPli() {
        if (session.selected == null) {
            status = "Select a private channel first."
            return
        }
        val now = SystemClock.elapsedRealtime()
        if (
        ...
        if (state.waiting(now, ackWait * 1000L) != null) {
            status =
                "Previous PLI is awaiting a receipt. Wait for confirmation or the ACK deadline before sending again."
            render()
            return
        }
        try {
            val (point, fix) = positionFix()
            var id = random.nextInt().toLong() and 0xffffffffL
            while (id == 0L) id = random.nextInt().toLong() and 0xffffffffL
            var name = map.deviceCallsign.ifBlank { "Relay" }
            while (name.toByteArray(Charsets.UTF_8).size > 40) name =
                name.substring(0, name.offsetByCodePoints(name.length, -1))
            ...
            positionIssue = e.message ?: "No fresh GPS fix; PLI paused."
            status = positionIssue!!
            render()
        }
    }
    ...
    private fun transmit(
        ...
        if (!active || session.selected == null || session.snapshot == null) {
            done("Choose a connected channel.")
            return
        }
        if (textTransport && bytes.size > TransportWire.MAX_BINARY) {
            done(
                "Text transport supports a shorter chat. Use at most 107 UTF-8 text bytes with a 24-byte callsign; no message was sent."
            )
            return
        }
        val now = SystemClock.elapsedRealtime()
        ...
        val key =
            when {
                ...
                ChatWire.isChat(bytes) ->
                    when (val m = ChatWire.decode(bytes)) {
                        is ChatWire.Text -> {
                            recipient = "!" + m.recipient.toString(16).padStart(8, '0')
                            "chat:${m.id}"
                        }
                        is ChatWire.Receipt -> {
                            receipt = true
                            "chat-ack:${m.recipient}:${m.id}"
                        }
                    }
                PointWire.isPointMessage(bytes) ->
                    when (val m = PointWire.decode(bytes)) {
                        is PointWire.Point -> {
                            recipient = "!" + m.point.recipient.toString(16).padStart(8, '0')
                            "point:${m.point.token}"
                        }
                        is PointWire.Receipt -> {
                            receipt = true
                            "point-ack:${m.recipient}:${m.token}"
                        }
                    }
                else -> {
                    receipt = true
                    "pli-ack:${(PliWire.decode(bytes) as PliWire.Receipt).id}"
                }
            }
        check(destination == DataPacket.ID_BROADCAST)
        if (!delivery.offer(key, bytes, recipient, now, receipt)) {
            done("Radio outbox full. Wait for pending sends.")
            return
        }
    ...
    private fun sendFrame(frame: DeliveryQueue.Frame) {
        ...
        worker.execute {
            ...
            val result = runCatching {
                val current = read(s)
                check(!ProfileCatalog.read(host).switching) {
                    "Channel activation is pending verification."
                }
                check(current == before && privateChannel(current.channels[index])) {
                ...
                s.send(
                    DataPacket(
                        ...
                        dataType = if (useText) 1 else PliWire.PORT,
                        id = packetId,
                        channel = index,
                        hopLimit = current.hops,
                        wantAck = !frame.receipt,
                    )
                )
            }
            handler.post {
                ...
                radioStatus =
                    if (error == null)
                        "Submitted via ${if (useText) "text" else "private data"} · attempt ${frame.attempt}"
                    else "Submission error · $error"
                status =
                    if (error == null) "Waiting for teammate confirmation · Recovery is automatic."
                    else "$error · Recovery pending."
                render()
            }
        }
    }
    ...
    private fun updateMarkers(now: Long) {
        markers.keys
            .filter { it !in state.peers }
            .toList()
            .forEach { markers.remove(it)?.let { m -> map.rootGroup.removeItem(m) } }
        state.peers.forEach { (node, peer) ->
            val p = peer.pli
            val marker =
                markers.getOrPut(node) {
            ...
            marker.title =
                "${p.callsign} [mesh ${if (peer.stale(now)) "STALE" else "recent"} ${peer.age(now)}s]"
            marker.setMetaString("callsign", marker.title)
            marker.setMetaString(
                "remarks",
                "Mesh-only last-known position. Fix ${Date(p.fixTime)}; received ${Date(peer.receivedWall)}. Not proof of current location.",
            )
        }
    ...
    private fun text(parent: LinearLayout, value: String, size: Float = 15f, color: Int = ink) =
        TextView(host).apply {
            text = value
            textSize = size
            setTextColor(color)
        ...
        column().apply {
            background =
                android.graphics.drawable.GradientDrawable().apply {
                    setColor(android.graphics.Color.rgb(25, 35, 42))
    ...
    private fun spinner(
        ...
        val widget =
            Spinner(host).apply { setBackgroundColor(android.graphics.Color.rgb(44, 67, 73)) }
        val adapter =
            object : ArrayAdapter<String>(host, android.R.layout.simple_spinner_item, labels) {
                override fun getView(
                    ...
                    super.getView(position, convertView, parent).apply {
                        (this as TextView).setTextColor(ink)
                ...
                override fun getDropDownView(
                    ...
                    super.getDropDownView(position, convertView, parent).apply {
                        (this as TextView).setTextColor(ink)
                        setBackgroundColor(android.graphics.Color.rgb(25, 35, 42))
                        isSingleLine = false
                        ellipsize = null
                        textSize = 15f
                        minHeight = dp(48)
            ...
            object : AdapterView.OnItemSelectedListener {
                ...
                override fun onItemSelected(
                    ...
                    if (position != lastChoice) {
                        lastChoice = position
                        if (!syncingControls) choose(position)
                    }
                }
            }
        ...
        if (pane == null) {
            body =
                column().apply {
                    ...
                    setBackgroundColor(android.graphics.Color.rgb(13, 21, 27))
                }
            val layout = body!!
            text(layout, "HARDLINE  /  RELAY", 18f, accent).typeface =
                android.graphics.Typeface.DEFAULT_BOLD
            val control = card(layout)
            val choices = column()
            control.addView(choices)
            val channels = column()
            choices.addView(channels, LinearLayout.LayoutParams(-1, -2))
            val reporting = column()
            choices.addView(reporting, LinearLayout.LayoutParams(-1, -2))
            text(channels, "CHANNEL", 11f, muted)
            channelButtons = column()
            channels.addView(channelButtons)
            val fallback =
                Switch(host).apply {
                    text = "Text transport"
                    setTextColor(ink)
                    isChecked = textTransport
                    setOnCheckedChangeListener { toggle, enabled ->
                        if (enabled && !delivery.fits(TransportWire.MAX_BINARY)) {
                            toggle.isChecked = false
                            status =
                                "A long chat is pending. Let it finish before enabling text transport."
                        } else {
                            textTransport = enabled
                            preferences.edit().putBoolean("textTransport", enabled).apply()
                            status =
                                if (enabled)
                                    "Text transport enabled for every outbound packet, including receipts."
                                else "Compact private-data transport enabled."
                        }
                        render()
                    }
                }
            ...
            text(
                reporting,
                "Higher-priority channel text · More airtime. Teammates receive either mode.",
                12f,
                muted,
            )
            ...
            intervalSpinner =
                spinner(
                    ...
                    PliWire.intervals.indexOf(interval),
                ) { selected ->
                    ...
                    if (seconds == 0 || session.selected != null) {
                        ...
                        if (seconds == 0) {
                            delivery.cancelPli()
                            state.latest?.let { state.failed(it.id, "Position sending paused") }
                        }
                        nextSend =
                            SystemClock.elapsedRealtime() + seconds * 1000L + random.nextInt(1000)
                        status =
                            if (seconds == 0) "Automatic PLI paused."
                            else "Reporting every ${seconds}s."
                    } else status = "Select a connected channel first."
                    render()
                }
            text(
                reporting,
                "Delivery recovery is automatic. Positions remain last-known until a fresh update arrives.",
                12f,
                muted,
            )
            val actions = LinearLayout(host)
            control.addView(actions)
            addButton(actions, "Send PLI now") { sendPli() }
            addButton(actions, "Pause PLI") {
                interval = 0
                manualRequested = false
                delivery.cancelPli()
                state.latest?.let { state.failed(it.id, "Position sending paused") }
                status = "Position sending paused."
                render()
            }
            ...
            text(
                summary,
                "Chat from ATAK's normal contact conversation · 160 UTF-8 bytes normally; use 107 or fewer with text transport · Private channel only",
                12f,
                muted,
            )
            retryButton = addButton(summary, "Retry last point") { points.retry() }
            peerText = text(summary, "", 13f)
            addButton(summary, "Connection details") {
                showDetails = !showDetails
                render()
            }
            diagnostics = text(summary, "", 13f, muted)
            addButton(summary, "Reconnect / refresh") {
                if (service == null) {
                    disconnect()
                    connect()
                }
            ...
            pane =
                PaneBuilder(
                    ...
                    .setMetaValue(Pane.PREFERRED_WIDTH_RATIO, 0.65)
                    .setMetaValue(Pane.PREFERRED_HEIGHT_RATIO, 1.0)
                    .build()
            rebuildChannels()
        }
        render()
            ...
            Button(host).apply {
                ...
                backgroundTintList =
                    android.content.res.ColorStateList.valueOf(
                        android.graphics.Color.rgb(44, 67, 73)
                    )
                setOnClickListener { action() }
            }
        layout.addView(
            widget,
            if (layout.orientation == LinearLayout.HORIZONTAL) LinearLayout.LayoutParams(0, -2, 1f)
    ...
    private fun chooseChannel(index: Int) {
        if (profileSwitching || catalogIssue != null) {
            status = catalogIssue ?: "Finish or verify channel activation in Relay first."
            rebuildChannels()
            render()
            return
        }
    ...
    private fun openProfiles(id: String? = null) {
        try {
            ...
            status = "Install Hardline Relay to manage channels."
            render()
        }
    }
    ...
    private fun matchesActive(entry: ProfileCatalog.Entry): Boolean {
        ...
        return entry.activation.isNotEmpty() &&
            current.node ==
                "!" + java.lang.Integer.toUnsignedString(entry.node, 16).padStart(8, '0') &&
            current.lora.channel_num == entry.slot &&
            current.lora.region == Config.LoRaConfig.RegionCode.US &&
            current.lora.use_preset &&
            current.lora.modem_preset == Config.LoRaConfig.ModemPreset.LONG_FAST &&
            current.lora.override_frequency == 0f &&
            current.lora.frequency_offset == 0f &&
    ...
    private fun rebuildChannels() {
        ...
        val labels =
            listOf("Select a channel") +
                installed.map { it.second } +
                saved.map { "${it.name} · ${if (it.locked) "Locked" else "Saved"}" }
        val selected =
            installed
                .indexOfFirst { it.first == session.selected }
    ...
    private fun age(ms: Long): String {
        ...
        else if (seconds < 3600) "${seconds / 60}m" else "${seconds / 3600}h"
    }
    ...
    private fun render() {
        ...
        chat.tick(now)
        chatText?.text = chat.status
        val p = points.last
        val gps = runCatching { positionFix().second }
        positionIssue = gps.exceptionOrNull()?.message
        gpsText?.text =
            gps.getOrNull()?.let {
                "GPS · Fix ${age((SystemClock.elapsedRealtimeNanos() - it.elapsedRealtimeNanos) / 1_000_000)} old"
            } ?: "GPS unavailable · No position sent"
        retryButton?.visibility =
            if (p.status in listOf(PointSendState.Status.UNCONFIRMED, PointSendState.Status.FAILED))
        ...
        pointText?.setTextColor(
            when (p.status) {
                PointSendState.Status.RECEIVED -> accent
                PointSendState.Status.FAILED -> android.graphics.Color.rgb(255, 130, 130)
                PointSendState.Status.AWAITING,
                PointSendState.Status.UNCONFIRMED -> android.graphics.Color.rgb(255, 195, 100)
                else -> ink
            }
        )
        val recent =
            state.peers.values.any { !it.stale(now) } ||
                (state.lastConfirmation >= 0 &&
                    now - state.lastConfirmation < maxOf(60_000L, interval * 3000L)) ||
                (lastPointProof >= 0 && now - lastPointProof < 60_000L) ||
                (chat.lastProof >= 0 && now - chat.lastProof < 60_000L)
        val health =
            if (profileSwitching || catalogIssue != null) MeshHealth.State.WAITING
            else
                MeshHealth.assess(
                    radioAvailable && service != null,
                    session.selected != null,
                    recent,
                    interval > 0 && positionIssue != null,
                )
        ...
        healthText?.text =
            when (health) {
                ...
                MeshHealth.State.WAITING ->
                    if (catalogIssue != null) "Open Relay to connect channels"
                    else if (profileSwitching) "Channel activation needs verification"
                    else if (positionIssue != null && interval > 0)
                        "Position reporting needs attention"
                    else "Waiting for mesh contact"
                MeshHealth.State.CONFIRMED -> "Recent mesh contact"
            }
        val name =
            session.selected?.let { session.snapshot?.channels?.getOrNull(it)?.name } ?: "None"
        statusText?.text =
            "$name · ${if (textTransport) "Text transport" else "Private data"} · ${if (interval == 0) "PLI off" else "PLI every ${interval}s"}"
        ...
        myPliText?.text = buildString {
            ...
            state.pending.values
                .toList()
                .takeLast(if (showDetails) 4 else 0)
                .filter { it.id != sent?.id }
                .reversed()
                .forEach { append("\nPrevious ${shortId(it.id)} · ${pliAttempt(it, now)}") }
            append("\nLast confirmed · ")
                .append(
                    if (state.lastConfirmation >= 0) "${age(now - state.lastConfirmation)} ago"
                    else "None this session"
                )
            if (showDetails) {
        ...
        peerText?.text = buildString {
            ...
            state.peers.values
                .sortedByDescending { it.stale(now) }
                .forEach { peer ->
                    append(
        ...
        val problem =
            health in listOf(MeshHealth.State.FAULT, MeshHealth.State.WAITING) ||
                p.status in
                    listOf(PointSendState.Status.FAILED, PointSendState.Status.UNCONFIRMED) ||
                positionIssue != null ||
                sent?.failure != null ||
                catalogIssue != null ||
                profileSwitching
        diagnostics?.visibility =
            if (problem || showDetails) android.view.View.VISIBLE else android.view.View.GONE
        diagnostics?.text = buildString {
            ...
            if (p.status in listOf(PointSendState.Status.FAILED, PointSendState.Status.UNCONFIRMED))
                append("\n").append(p.detail)
            if (
                health == MeshHealth.State.WAITING &&
                    !recent &&
                    !profileSwitching &&
                    catalogIssue == null
            )
        ...
        if (intervalSpinner?.selectedItemPosition != mode)
            intervalSpinner?.setSelection(mode, false)
        syncingControls = false
    }

    private fun pliAttempt(sent: PliState.Sent, now: Long) =
        when {
            sent.receipts.isNotEmpty() ->
                "Confirmed by ${sent.receipts.size} · sent ${age(now - sent.at)} ago"
