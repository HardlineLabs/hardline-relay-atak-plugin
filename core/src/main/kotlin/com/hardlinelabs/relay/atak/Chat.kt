...
object ChatWire {
    ...
    sealed interface Message { val id: UUID; val recipient: Long }
    data class Text(override val id: UUID, override val recipient: Long, val time: Long,
                    val callsign: String, val text: String) : Message
    data class Receipt(override val id: UUID, override val recipient: Long) : Message
    fun isChat(bytes: ByteArray) = bytes.size >= 5 && ByteBuffer.wrap(bytes).int == MAGIC && bytes[4].toInt() in 5..6
    fun encode(message: Message): ByteArray {
        require(message.recipient in 1..0xfffffffeL)
        val name = if (message is Text) message.callsign.toByteArray(Charsets.UTF_8) else byteArrayOf()
        val body = if (message is Text) message.text.toByteArray(Charsets.UTF_8) else byteArrayOf()
        if (message is Text) {
            require(message.time > 0 && name.size in 1..24 && message.callsign.none { it.isISOControl() })
            require(body.size in 1..MAX_TEXT_BYTES && message.text.isNotBlank() &&
        ...
        b.putInt(MAGIC).put(if (message is Text) 5.toByte() else 6.toByte())
            .putLong(message.id.mostSignificantBits).putLong(message.id.leastSignificantBits).putInt(message.recipient.toInt())
    ...
    fun decode(bytes: ByteArray): Message {
        ...
        val result = if (type == 6) { require(!b.hasRemaining()); Receipt(id, recipient) } else {
    ...
    private fun utf8(bytes: ByteArray) = Charsets.UTF_8.newDecoder()
