...
object PliWire {
    ...
    fun encode(p: Pli): ByteArray {
        ...
        return ByteBuffer.allocate(35 + name.size)
            .putInt(MAGIC)
            .put(1)
            .putLong(p.id)
            .putLong(p.fixTime)
    ...
    fun compact(p: Pli): ByteArray {
        ...
        val name = p.callsign.toByteArray(Charsets.UTF_8)
        return ByteBuffer.allocate(22 + name.size)
            .putInt(MAGIC)
            .put(7)
            .putInt(p.id.toInt())
            .putInt((p.fixTime / 1000).toInt())
            .putInt((p.lat * 1e7).roundToInt())
            .putInt((p.lon * 1e7).roundToInt())
            .put(intervals.indexOf(p.interval).toByte())
            .put(name)
            .array()
    }
...
class PliState {
    data class Peer(val pli: Pli, val receivedAt: Long, val receivedWall: Long) {
        ...
        fun stale(now: Long) =
            now - receivedAt + (receivedWall - pli.fixTime).coerceAtLeast(0) >=
                maxOf(60_000L, pli.interval * 3_000L)
    }

    data class Sent(
        val id: Long,
        val at: Long,
        val receipts: MutableSet<String> = linkedSetOf(),
    ...
    val unanswered
        get() = pending.values.count { it.deadlineReached && it.receipts.isEmpty() }
    ...
    fun waiting(now: Long, waitMillis: Long): Sent? {
        ...
        if (attempt.receipts.isNotEmpty() || attempt.failure != null || attempt.deadlineReached)
            return null
        if (now - attempt.at >= waitMillis) {
    ...
    fun receipt(id: Long, from: String, now: Long): Boolean {
        ...
        return sent.receipts.add(from).also {
            if (it) {
    ...
    fun failed(id: Long, reason: String) {
        pending[id]?.takeIf { it.receipts.isEmpty() }?.failure = reason
    }
    ...
    var lastReception = Reception.ACCEPTED
        private set
    ...
    fun accept(from: String, p: Pli, now: Long, wall: Long): Boolean {
        ...
        if (seen.containsKey(key)) {
            lastReception = Reception.DUPLICATE
            return false
        }
        val previous = peers[from]
        // A delayed older fix must not move a marker backwards or refresh its age.
        if (p.fixTime > wall + 30_000) {
            lastReception = Reception.FUTURE
            return false
        }
        if (wall - p.fixTime > 900_000) {
            lastReception = Reception.EXPIRED
            return false
        }
        if (previous != null && p.fixTime < previous.pli.fixTime) {
            lastReception = Reception.OLDER
            return false
        }
        seen[key] = now
