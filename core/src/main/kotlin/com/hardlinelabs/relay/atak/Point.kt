...
object PointWire {
    ...
    fun isPointMessage(bytes: ByteArray): Boolean =
        bytes.size >= 5 && ByteBuffer.wrap(bytes).int == MAGIC && bytes[4].toInt() in 3..4
    ...
    fun pointId(uid: String): Long {
        ...
        return ByteBuffer.wrap(
                MessageDigest.getInstance("SHA-256").digest(uid.toByteArray(Charsets.UTF_8))
            )
            .long
    ...
    fun encode(p: MeshPoint): ByteArray {
        ...
        require(
            p.name.isNotBlank() &&
                p.name.none { it.isISOControl() } &&
                name.size in 1..MAX_NAME_BYTES
        ) {
        ...
        return ByteBuffer.allocate(47 + name.size)
            .putInt(MAGIC)
            .put(3)
            .putLong(p.token)
            .putLong(p.id)
            .putLong(p.revision)
            .putInt(p.recipient.toInt())
            .putInt((p.lat * 1e7).roundToInt())
            .putInt((p.lon * 1e7).roundToInt())
            .put(p.symbol.toByte())
            .putInt(p.color)
    ...
    fun decode(bytes: ByteArray): Message {
        ...
        val point =
            MeshPoint(token, id, revision, recipient, lat, lon, symbol, color, decodeName(name))
        encode(point)
        return Point(point)
    }

    private fun decodeName(bytes: ByteArray) =
...
class PointSendState {
    ...
    var point: MeshPoint? = null
        private set

    var recipientName = ""
        private set

    var detail = "No point sent this session."
        private set
    ...
    fun clear() {
        ...
        point = null
        recipientName = ""
        detail = "No point sent this session."
    ...
    fun receipt(token: Long, from: Long, now: Long): Boolean {
        attempts[token]
            ?.takeIf { !it.confirmed && it.recipient == from && now - it.at in 0..300_000 }
            ?.let {
                it.confirmed = true
                roundTrips.add(now - it.at)
            }
        ...
        if (
            !receiptEligible ||
                p.token != token ||
                p.recipient != from ||
                now - startedAt !in 0..300_000 ||
                status !in listOf(Status.SUBMITTING, Status.AWAITING, Status.UNCONFIRMED)
        )
            return false
        status = Status.RECEIVED
        detail = "Recipient plugin created/updated the point."
        return true
    }
    ...
    fun tick(now: Long) {
        if (status in listOf(Status.SUBMITTING, Status.AWAITING) && now - startedAt >= 60_000) {
            ...
            detail = "No receipt after 60s; delivery is unknown."
        }
    }
    ...
    fun interrupted() {
        ...
        if (status in listOf(Status.SUBMITTING, Status.AWAITING)) {
            status = Status.UNCONFIRMED
            detail = "Connection/channel changed; delivery is unknown."
        }
    }
}
...
class PointReceiveState {
    ...
    fun committed(sender: Long, p: MeshPoint, now: Long) {
        received[Key(sender, p.id)] = Received(p, now)
    }
