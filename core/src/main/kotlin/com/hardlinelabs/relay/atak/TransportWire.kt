...
object TransportWire {
    ...
    fun text(bytes: ByteArray): ByteArray {
        require(bytes.size in 1..MAX_BINARY) { "Text transport packet too long. Shorten the chat message; positions and points already fit." }
    ...
    fun decodeText(bytes: ByteArray): ByteArray? {
        if (bytes.size < PREFIX.length || !bytes.copyOfRange(0, PREFIX.length).contentEquals(PREFIX.toByteArray(Charsets.US_ASCII))) return null
        ...
        require(decoded.size >= 5 && decoded.copyOfRange(0, 4).contentEquals(byteArrayOf(0x48, 0x52, 0x50, 0x31)))
        return decoded
    }
}
