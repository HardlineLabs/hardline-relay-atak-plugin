...
class PliTest {
    @Test
    fun intervalCannotOverrunAckWaitAndLateReceiptDoesNotReleaseNewerAttempt() {
        val s = PliState()
        s.sent(1, 0)
    ...
    @Test
    fun confirmationReleasesOverdueUpdateBeforeDeadline() {
        val s = PliState()
        s.sent(1, 0)
    ...
    @Test
    fun receiptRoundTrip() {
        assertEquals(PliWire.Receipt(42), PliWire.decode(PliWire.ack(42)))
    }
    ...
    fun malformedRejected() {
        for (bytes in
            listOf(
                byteArrayOf(),
                PliWire.ack(42) + 0,
                PliWire.encode(p).dropLast(1).toByteArray(),
                ByteArray(80),
            )) {
    ...
    fun invalidCoordinatesRejected() {
        assertThrows(IllegalArgumentException::class.java) {
            PliWire.encode(p.copy(lat = Double.NaN))
        }
    ...
    @Test
    fun lateAndFutureFixesRejected() {
        val s = PliState()
    ...
    @Test
    fun olderFixCannotMoveMarkerBackwards() {
        val s = PliState()
        assertTrue(s.accept("peer", p, 0, 100_000))
    ...
    @Test
    fun fakeTwoWayTransportAndNoAckLoop() {
        val a = PliState()
        val b = PliState()
        a.sent(p.id, 0)
    ...
    @Test
    fun freshnessUsesMonotonicTime() {
        val s = PliState()
        s.accept("b", p.copy(interval = 30), 1000, 100_000)
    ...
    @Test
    fun historyBoundedAndResettable() {
        val s = PliState()
        repeat(100) {
            s.sent(it + 1L, 0)
            s.accept("p$it", p.copy(id = it + 1L), 0, 100_000)
        }
        assertEquals(32, s.pending.size)
        assertEquals(32, s.peers.size)
        s.expire(300_001)
    ...
    @Test
    fun submissionFailureDoesNotOverrideAProcessingReceipt() {
        val s = PliState()
        s.sent(42, 0)
        s.failed(42, "Radio disconnected")
