...
class PointTest {
    ...
    @Test
    fun boundedPointAndReceiptRoundTrip() {
    ...
    @Test
    fun wireRejectsInvalidAndOversizedInputs() {
        for (bad in
            listOf(
                p.copy(lat = Double.NaN),
                p.copy(lon = 181.0),
                p.copy(name = "x".repeat(25)),
                p.copy(name = " "),
                p.copy(name = "a\nb"),
                p.copy(name = "é".repeat(13)),
                p.copy(name = "\uD800"),
                p.copy(symbol = 255),
                p.copy(recipient = 0xffffffffL),
                p.copy(token = 0),
                p.copy(id = 0),
                p.copy(revision = 0),
            )) {
        ...
        for (bad in
            listOf(
                ...
                ByteArray(100),
                PointWire.receipt(7, 1) + 0,
                bytes.clone().apply { this[47] = 0xff.toByte() },
                bytes.clone().apply { this[4] = 99 },
                bytes.clone().apply { this[0] = 0 },
            )) {
            assertThrows(Exception::class.java) { PointWire.decode(bad) }
        }
        for (size in bytes.indices) assertThrows(Exception::class.java) {
            PointWire.decode(bytes.copyOf(size))
        }
    }

    @Test
    fun unicodeCoordinatesAndUnsignedNodeIds() {
    ...
    @Test
    fun onlyAddressedRecipientImportsAndOnlyIntendedPeerConfirms() {
        val sender = PointSendState()
        val receiver = PointReceiveState()
    ...
    @Test
    fun lastAttemptCannotBeOverwrittenByEarlierCallbacks() {
        val s = PointSendState()
        s.begin(p, "B", 0)
        s.begin(p.copy(token = 8), "B", 1000)
        s.submitted(7)
        s.failed(7, "old failure")
        ...
        s.submitted(8)
        assertEquals(PointSendState.Status.AWAITING, s.status)
        assertTrue(s.receipt(8, 2, 4000))
        assertEquals(2, s.roundTrips.count)
        assertEquals(2500L, s.roundTrips.average)
        s.interrupted()
        assertEquals(0, s.roundTrips.count)
    }

    @Test
    fun timeoutIsUnknownAndLateReceiptCanConfirmWithinHistory() {
        val s = PointSendState()
        s.begin(p, "B", 0)
        s.submitted(7)
        s.tick(59_999)
        ...
        s.tick(600_000)
        assertEquals(PointSendState.Status.RECEIVED, s.status)
        s.begin(p.copy(token = 8), "B", 0)
        assertFalse(s.receipt(8, 2, 300_001))
    }

    @Test
    fun resetDoesNotClaimLossOrEraseConfirmedResult() {
        val s = PointSendState()
        s.begin(p, "B", 0)
        s.interrupted()
        assertEquals(PointSendState.Status.UNCONFIRMED, s.status)
        ...
        s.clear()
        assertEquals(PointSendState.Status.NONE, s.status)
    }

    @Test
    fun duplicateCanRecoverLostReceiptWithoutReimporting() {
        ...
        assertEquals(
            PointReceiveState.Decision.RECEIPT_ONLY,
            r.decide(1, 2, p.copy(token = 8), 5100),
        )
    }

    @Test
    fun delayedUpdatesNeverMovePointBackwards() {
        val r = PointReceiveState()
        r.committed(1, p, 0)
        assertEquals(
            PointReceiveState.Decision.IGNORE,
            r.decide(1, 2, p.copy(revision = 99_999), 10_000),
        )
        assertEquals(PointReceiveState.Decision.IGNORE, r.decide(1, 2, p.copy(lat = 41.0), 10_000))
        assertEquals(
            PointReceiveState.Decision.IMPORT,
            r.decide(1, 2, p.copy(revision = 100_001, lat = 41.0), 10_000),
        )
    }

    @Test
    fun receiveHistoryHasAHardBoundAndIndependentSenders() {
        val r = PointReceiveState()
        ...
        assertEquals(
            PointReceiveState.Decision.IMPORT,
            r.decide(1, 2, p.copy(revision = 100_001), 10_000),
        )
        r.clear()
