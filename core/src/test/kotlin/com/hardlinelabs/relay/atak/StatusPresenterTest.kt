...
class StatusPresenterTest {
    @Test fun contractVectorsMatch() {
        File("../protocol/status-v1.tsv").readLines().filter { !it.startsWith("#") && it.isNotBlank() }
            .forEach {
                val row = it.split('\t')
                assertEquals(row[3], StatusPresenter.label(row[0].toInt(), row[1], row[2]))
            }
    }
    @Test fun takServerCannotMakeMeshAppearConnected() {
    ...
    @Test fun unknownModeFailsClosed() {
