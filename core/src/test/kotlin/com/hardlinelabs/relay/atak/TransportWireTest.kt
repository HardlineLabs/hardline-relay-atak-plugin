...
class TransportWireTest {
    @Test
    fun textRoundTripsAllPacketFamiliesAndOrdinaryChatIsIgnored() {
        ...
        for (bytes in
            listOf(
                ...
                PliWire.encode(p),
                PliWire.ack(1),
                PointWire.encode(point),
                PointWire.receipt(1, 3),
                ChatWire.encode(chat),
                ChatWire.encode(ChatWire.Receipt(chat.id, 3)),
            )) {
    ...
    @Test
    fun boundsInvalidEncodingAndNoFragmentation() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            TransportWire.decodeText("HLR1:???".toByteArray())
        }
        assertThrows(IllegalArgumentException::class.java) {
            TransportWire.decodeText("HLR1:".toByteArray())
        }
        assertThrows(IllegalArgumentException::class.java) {
            TransportWire.decodeText(ByteArray(226).apply { "HLR1:".toByteArray().copyInto(this) })
        }
        val q = DeliveryQueue { 0 }
        q.offer("chat:1", ByteArray(218), "peer", 0)
        assertFalse(q.fits(165))
        q.clear()
        assertTrue(q.fits(165))
    }
}
